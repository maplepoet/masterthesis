...
void Message::release_message(Message *msg)
{
	...
	case FORWARD_REQ:{
		ForwardRequests *m_msg = (ForwardRequests *)msg;
		m_msg->release();
		delete m_msg;
		break;
	}
...
bool ClientResponseMessage::validate()
{
	...
	//ADD
	if (this->return_node_id >= g_node_cnt + g_client_node_cnt){
		return true;
	}
...
#else
	// assert(get_current_view(thd_id) == g_node_id || g_node_id == 5 || g_node_id == 9 || g_node_id == 13 || g_node_id == 17);
#endif
...
bool BatchRequests::validate(uint64_t thd_id)
{
	...
	if (g_node_id > g_node_cnt) printf("validate lower layer");
	fflush(stdout);
...
// Forward Requests
void ForwardRequests::copy_from_buf(char *buf){
	Message::mcopy_from_buf(buf);
...
void ForwardRequests::copy_to_buf(char *buf){
	Message::mcopy_to_buf(buf);
...
void ForwardRequests::copy_from_txn(TxnManager *txn){
	assert(0);
}
void ForwardRequests::copy_to_txn(TxnManager *txn){
	Message::mcopy_to_txn(txn);
	assert(0);
}
uint64_t ForwardRequests::get_size(){
	uint64_t size = Message::mget_size();
...
void ForwardRequests::init(){
	this->return_node = g_node_id;
	this->batch_size = get_batch_size();
	this->cqrySet.init(get_batch_size());
}

void ForwardRequests::release(){
	for (uint64_t i = 0; i < get_batch_size(); i++)
	{
		Message::release_message(cqrySet[i]);
	}
...
void ForwardRequests::sign(uint64_t dest_node){
	#if USE_CRYPTO
	string message = this->getString();
	signingClientNode(message, this->signature, this->pubKey, dest_node);
...
string ForwardRequests::getString(){
	string message = std::to_string(this->return_node);
	for (int i = 0; i < BATCH_SIZE; i++)
	{
		message += cqrySet[i]->getRequestString();
