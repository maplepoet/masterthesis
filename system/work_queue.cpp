...
void QWorkQueue::init()
{
    ...
    rsp_queue = new boost::lockfree::queue<work_queue_entry *>(0);
...
void QWorkQueue::enqueue(uint64_t thd_id, Message *msg, bool busy)
{
    ...
    if (msg->rtype == CL_QRY || msg->rtype == CL_BATCH || msg->rtype == FORWARD_REQ)
    {
    ...
    else if (msg->rtype == PBFT_CHKPT_MSG)
    {
        ...
        printf("RSP queue add CL_RSP. \n");
        fflush(stdout);
        push_to_queue(entry, rsp_queue);
    }
#if GBFT
...
Message *QWorkQueue::dequeue(uint64_t thd_id)
{
...
#else
    assert(g_thread_cnt == g_worker_thread_cnt + g_batching_thread_cnt + g_checkpointing_thread_cnt + g_execution_thread_cnt + g_rsp_thread_cnt);
#endif
    // Worker Threads
    ...
    // RSP Threads
    else if (thd_id < g_worker_thread_cnt + g_batching_thread_cnt + g_checkpointing_thread_cnt + g_execution_thread_cnt + g_rsp_thread_cnt)
    {
        valid = rsp_queue->pop(entry);
    }
    ...
    {
        message = entry->msg;
        if (message->rtype == CL_RSP){
            printf("CL_RSP get. \n");
            fflush(stdout);
        }
        if (message->rtype == FORWARD_REQ){
            printf("forward get. \n");
            fflush(stdout);
        }
        assert(message);
