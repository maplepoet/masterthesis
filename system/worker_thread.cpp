...
void WorkerThread::send_key()
{
    ...
    for (uint64_t i = 0; i < g_node_cnt + g_client_node_cnt + g_repl_cnt * g_node_cnt; i++)
    {
        if (i == g_node_id)
        {
            continue;
...
#if Hierarchical   
    case CL_RSP:
        // printf("Process CL_RSP msg %ld. \n", msg->get_txn_id());
        // fflush(stdout);
        rc = process_rsp_msg(msg);
        break;
#endif
#if VIEW_CHANGES
    case VIEW_CHANGE:
...
void WorkerThread::store_batch_msg(BatchRequests *breq)  // store batch msg
{
...
RC WorkerThread::run()
{
    ...
    while (!simulation->is_done())
    {
        ...
        if (msg->rtype != BATCH_REQ && msg->rtype != CL_BATCH && msg->rtype != EXECUTE_MSG && msg->rtype != CL_RSP)
        {
...
RC WorkerThread::process_execute_msg(Message *msg)
{
...
#if Hierarchical
    // send CL_Batch to lower layer replcias.
    if (g_node_id < g_node_cnt && l_node_cnt[g_node_id] > 0) {
        Message *mssg = Message::create_message(CL_BATCH);
        ClientQueryBatch *bmsg = (ClientQueryBatch *)mssg;
        create_and_send_client_upper(bmsg,emsg->index);
    }
    ...
    // send CL_RSP to clients
        ...
        if (g_node_id >= g_node_cnt + g_client_node_cnt) {

        vector<uint64_t> dest;
        dest.push_back(find_cluster());
        msg_queue.enqueue(get_thd_id(), crsp, dest);
...
#if Shard
        // Transfer the clqry to next shard

        // 1. Get to know where the shard is, and who is the next shard.
        // 2. Define the messgae it will send to the next shard
        // 3. Forward message
        // 4. 

        send_forward_msg(msg);
    ...
    // if (g_node_id >= g_node_cnt + g_client_node_cnt || (g_node_id < g_node_cnt && l_node_cnt[g_node_id]==0)) {
        // send_checkpoints(txn_man->get_txn_id());
    // }

    // Setting the next expected prepare message id.
    set_expectedExecuteCount(get_batch_size() + emsg->txn_id);
...
void WorkerThread::create_and_send_batchreq(ClientQueryBatch *msg, uint64_t tid)
{
    ...
    if (g_node_id >= g_node_cnt + g_client_node_cnt) {
        uint64_t m = find_cluster();
        uint64_t startnode = cal_startnode(m);
        
        // printf("lower layer send pre-pare. \n ");
        // fflush(stdout);
        for (uint64_t i = startnode; i < startnode + l_node_cnt[m]; i++)
        {
...
uint64_t WorkerThread::cal_startnode(uint64_t clusternum){
    uint64_t start_node = g_node_cnt + g_client_node_cnt;
	for (uint64_t i = 0;i < clusternum; i++)
