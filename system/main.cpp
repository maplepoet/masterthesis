...
int main(int argc, char *argv[])
{
  ...
  */
    ...
    for (unsigned int i = 0; i < g_total_node_cnt; i++)
    {
        cmacPrivateKeys[i] = CmacGenerateHexKey(16);
        printf("Node %d", g_node_id);
        fflush(stdout);
        if (i == g_node_id && g_node_id == g_total_node_cnt-1) break;
        if (i == g_node_id)
        {
            continue;
        }
    ...
    cout << all_thd_cnt << " " << g_this_total_thread_cnt << endl;
    fflush(stdout);
    ...
    uint64_t id = 0;
    for (uint64_t i = 0; i < wthd_cnt - 1; i++)  // 0,1,2,3
    {
    ...
    for (uint64_t j = 0; j < sthd_cnt; j++) // output thread
    {
    ...
    for (uint64_t i = 0; i < g_send_thread_cnt; i++)   // IO thread
        printf("Output %ld: %f\n", i + g_thread_cnt + g_rem_thread_cnt, output_thd_idle_time[i] / BILLION);
    for (uint64_t i = 0; i < g_rem_thread_cnt; i++)    // IO thread
