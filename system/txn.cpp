...
void TxnManager::send_pbft_prep_msgs()
{
        ...
        for (uint64_t i = startnode; i < startnode + l_node_cnt[m]; i++)
    {
        ...
        printf("lower layer send prepare to %ld \n", i);
        fflush(stdout);
        dest.push_back(i);
    }
    }
...
void TxnManager::send_pbft_commit_msgs()
{
        ...
        for (uint64_t i = startnode; i < startnode + l_node_cnt[m]; i++)
    {
        ...
        printf("lower layer send commit. %ld \n", i);
        fflush(stdout);
        dest.push_back(i);
    }
    }
...
uint64_t TxnManager::find_cluster(){
    int m;
...
uint64_t TxnManager::cal_startnode(uint64_t clusternum){
