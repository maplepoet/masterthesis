...
RC ClientThread::run()
{
	...
	uint32_t next_node_id = get_client_view(); // TO CHANGE, bc now for each batch not the first node to send.
	while (!simulation->is_done())
	{
		heartbeat();
		progress_stats();
		int32_t inf_cnt;
		uint32_t next_node = get_client_view(); // TO CHANGE, bc now for each batch not the first node to send.

#if GBFT
		next_node_id = view_to_primary(get_client_view(get_cluster_number()));
#else
		next_node_id = get_client_view(); // TO CHANGE, bc now for each batch not the first node to send.
#endif
