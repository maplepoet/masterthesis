...
// lower layer replicas
uint64_t l_node_cnt[NODE_CNT] = {4,4,4,4}; 
...
UInt32 g_rsp_thread_cnt = RSP_THREAD_CNT;
...
string g_pub_keys[NODE_CNT + CLIENT_NODE_CNT + REPLICA_CNT * NODE_CNT]; //stores public keys of other nodes.
//CMAC
string cmacPrivateKeys[NODE_CNT + CLIENT_NODE_CNT + REPLICA_CNT * NODE_CNT ];
string cmacOthersKeys[NODE_CNT + CLIENT_NODE_CNT + REPLICA_CNT * NODE_CNT ];
//ED25519
CryptoPP::ed25519::Verifier verifier[NODE_CNT + CLIENT_NODE_CNT + REPLICA_CNT * NODE_CNT ];
CryptoPP::ed25519::Signer signer;

// Receiving keys
uint64_t receivedKeys[NODE_CNT + CLIENT_NODE_CNT + REPLICA_CNT * NODE_CNT ];
...
#if Shard
uint64_t g_min_invalid_nodes = (shard_cluster_size - 1) / 3;
#elif GBFT
...
#if Shard
UInt32 shard_cluster_size = Shard_CLUSTER_SIZE;
UInt32 shard_cluster_cnt = NODE_CNT / Shard_CLUSTER_SIZE;
// UInt32 shard_commit_certificate_thread_cnt = Shard_CCM_THREAD_CNT;

uint32_t g_view[NODE_CNT / SHARD_CLUSTER_SIZE] = {0};
std::mutex viewMTX[NODE_CNT / SHARD_CLUSTER_SIZE];
...
uint64_t get_cluster_number(uint64_t i)
{
	if (i >= g_node_cnt && i < g_node_cnt + g_client_node_cnt)
	{
		...
		return (client_number / (g_client_node_cnt / shard_cluster_cnt));
	}
	else
		return (i / shard_cluster_size);
}

uint64_t view_to_primary(uint64_t view, uint64_t node)
{
	return get_cluster_number(node) * shard_cluster_size + view;
}

int is_in_same_cluster(uint64_t first_id, uint64_t second_id)
{
	return (int)(first_id / shard_cluster_size) == (int)(second_id / shard_cluster_size);
}
bool is_local_request(TxnManager *tman)
{
	...
	int primary = client_number / (g_client_node_cnt / shard_cluster_cnt);
	if (is_in_same_cluster(primary * shard_cluster_size, g_node_id))
	{
		return true;
	}
	return false;
...
SpinLockMap<uint64_t, uint64_t> rep_responses_count[NODE_CNT];
SpinLockMap<uint64_t, ClientResponseMessage *> rep_responses_directory[NODE_CNT];
