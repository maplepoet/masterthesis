...
RC InputThread::client_recv_loop()
{
    ...
    while (!simulation->is_done())
    {
        ...
        while (!msgs->empty())
        {
            ...
            if (msg->rtype != CL_RSP)  //error if not clientresponse message
            {
            ...
            printf("client receive CL_RSP %ld from %ld. \n", msg->get_txn_id(),msg->return_node_id);
            fflush(stdout);
    ...
    while (!simulation->is_done())
    {
        ...
        while (!msgs->empty())
        {
            ...
            if (msg->rtype == CL_RSP){
                ...
                printf("replicas receive CL_RSP %ld. \n", msg->txn_id);
                fflush(stdout);

                uint64_t m = g_node_id;
                ...
                if (rep_responses_directory[m].exists(msg->txn_id))
                {
                    ClientResponseMessage *old_clrsp = rep_responses_directory[m].get(msg->txn_id);
                    response_count = rep_responses_count[m].get(msg->txn_id);
                    response_count++;
                    for (uint64_t j = 0; j < get_batch_size(); j++)
                    {
                        if (old_clrsp->index[j] != clrsp->index[j])
                    ...
                    rep_responses_count[m].add(msg->txn_id, response_count);
                }

                else if (!rep_responses_count[m].exists(msg->txn_id))
                {
                    rep_responses_count[m].add(msg->txn_id, 1);
                    char *buf = create_msg_buffer(msg);
                    Message *deepMsg = deep_copy_msg(buf, msg);
                    delete_msg_buffer(buf);
                    rep_responses_directory[m].add(msg->txn_id, (ClientResponseMessage *)deepMsg);
                }

               if (response_count ==  2 * g_min_invalid_nodes + 1)
            {
                work_queue.enqueue(get_thd_id(), msg, false);

#if CLIENT_RESPONSE_BATCH == true
                rep_responses_directory[m].remove(msg->txn_id);
#endif
            }
            }
