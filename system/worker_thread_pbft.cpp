...
#if CONSENSUS == PBFT && !GBFT && !Shard
/**
 * Processes an incoming client batch and sends a Pre-prepare message to all replicas.
 *
...
RC WorkerThread::process_client_batch(Message *msg)
{
    ...
    //     printf("Receive cl_batch %ld. \n",msg->txn_id);
    //     fflush(stdout);
    // }
    // if (g_node_id < g_node_cnt){
    //     printf("Primary receice cl_batch %ld. \n",msg->txn_id);
    //     fflush(stdout);
    // }
...
RC WorkerThread::process_batch(Message *msg)
{
    ...
    // if (g_node_id >= g_node_cnt + g_client_node_cnt){
    //     printf("lower layer receive pre-prepare. \n");
    //     fflush(stdout);
    // }
    ...
    if (txn_man->is_prepared())
    {
        ...
        if (g_node_id >= g_node_cnt + g_client_node_cnt) {
            printf("lower layer send commit. \n");
            fflush(stdout);
        }
        ...
        if (txn_man->is_committed())
        {
            ...
            // if (g_node_id >= g_node_cnt+g_client_node_cnt) {
            //     printf("lower layer send execute. \n");
            //     fflush(stdout);
            // }
...
RC WorkerThread::process_pbft_prep_msg(Message *msg)
{
    ...
    if (g_node_id >= g_node_cnt + g_client_node_cnt) {
        printf("lower layer receive prepare. \n");
        fflush(stdout);
    }
...
bool WorkerThread::committed_local(PBFTCommitMessage *msg)
{
    ...
    //fflush(stdout);
    if (g_node_id >= g_node_cnt+g_client_node_cnt) {
        printf("lower layer receive commit. \n");
        fflush(stdout);
    }
