...
public:
    ...
    // upper layer replicas send batchreq to lower layer.
    ...
    RC send_forward_msg(Message *msg);
    RC process_forward_msg(Message *msg);
    void send_response_msg(Message *msg);
    void create_and_send_batchreq_shard(ForwardRequests *msg, uint64_t tid);
    // shard
    RC process_client_msg(Message *msg);
    RC process_response_msg(Message *msg);
    uint64_t find_shard();
...
#if Hierarchical
    // overlay_layer 1
    void internal_send_key_overlay();
    void create_and_send_client_upper(ClientQueryBatch *msg, uint64_t tid);
    // To get the primary node in the lower layer
    uint64_t get_server_view();
    RC process_rsp_msg(Message *msg);
    // overlay_layer 2
    void lower_send_key_overlay();
    uint64_t find_cluster();
    uint64_t cal_startnode(uint64_t clusternum);
#endif
